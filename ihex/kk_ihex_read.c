...
void
ihex_read_at_address (struct ihex_state * ihex, ihex_address_t address, cb_ihex_data_read_t cb_read, void *args) {
    ihex_begin_read(ihex, cb_read, args);
    ihex->address = address;
}

#ifndef IHEX_DISABLE_SEGMENTS
void
ihex_read_at_segment (struct ihex_state * ihex, ihex_segment_t segment, cb_ihex_data_read_t cb_read, void *args) {
    ihex_begin_read(ihex, cb_read, args);
