...
Layout:
    [header]
        0-3:    Tag='c'(1)    Ver='0'(1)    block size(2)
    [data content]
        the config content controlled by up layer app
    [tail]
        [last 4bytes]:  Fw crc(3)   rsv(1)
*/

bool cb_test_element_c0(int type)
{
...
static uint32_t cb_get_element_c0(/*config_container_t*/void *cfg_ptr, int type)
{
...
static config_body_c0_t *_read_body_value(const config_body_info_t *body, int type, int param)
{
    config_body_c0_t *body_c0 = (config_body_c0_t *)body->buf;
    int count = 0;
    
    switch (type) {
    case CB_CFG_BODY_ELEM_DATA:
        if (param >= 0 && param < body->elem_count) {
            return body_c0 + param;
        }
    default:
        return NULL;
    }
...
static void *cb_read_data_c0(/* config_container_t */void *cfg_ptr, int type, int param)
{
    config_container_t *cfg = (config_container_t *)cfg_ptr;
    int major = TO_OP(type);
...
static void cb_show_element_c0(/*config_container_t*/void *cfg_ptr)
{
    config_container_t *cfg = (config_container_t *)cfg_ptr;

	DBG_INFO(UPDI_DEBUG, "==========================");
    
    DBG(UPDI_DEBUG, "Config Block Content(c0):", (u8 *)cfg->head, cfg->head->data.size, "%02X ");
}

int cb_create_configure_block_c0(config_container_t *cfg, char *data, int len)
{
    config_header_t *hdr;
    config_crc_t * tail;
    unsigned short size;
    ...
    hdr->data.version.value = CONFIG_BLOCK_C0_VERSION;
    hdr->data.size = size;
    ...
    cfg->body_info.elem_count = (unsigned short)(len / sizeof(config_body_elem_c0_t));
    cfg->tail = (config_tail_t *)tail;
    cfg->tag.version.value = hdr->data.version.value;
    cfg->tag.type = BIT_MASK(MEM_ALLOC);
    cfg->intf.test = cb_test_element_c0;
    cfg->intf.get = cb_get_element_c0;
    cfg->intf.read = cb_read_data_c0;
    cfg->intf.show = cb_show_element_c0;

    return 0;
}

int cb_set_configure_block_ptr_c0(config_container_t *cfg, char *data, int len, unsigned short flag)
{
    //config_container_t *cfg = (config_container_t *)info_ptr;
    config_header_t * head = (config_header_t *)data;
    unsigned short size;

    size = (unsigned short)(sizeof(config_header_t) + sizeof(config_body_elem_c0_t) + sizeof(config_tail_t));
    ...
    cfg->body_info.elem_count = (unsigned short)(cfg->body_info.size / sizeof(config_body_elem_c0_t));
...
void cb_destory_c0(void *cfg_ptr)
{
    config_container_t *cfg = (config_container_t *)cfg_ptr;
