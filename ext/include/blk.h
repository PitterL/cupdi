...
typedef enum { B_HEAD = 1, B_VER, B_SIZE, B_CRC, B_DATA_TYPES } B_DTYPE;
typedef enum { IB_HEAD = B_HEAD, IB_FW_VER = B_DATA_TYPES, IB_FW_SIZE, IB_FUSE, IB_CFG, IB_CRC, IB_REG, IB_DATA_TYPES } IB_DTYPE;
typedef enum { CB_HEAD = B_HEAD, CB_BODY = IB_DATA_TYPES, CB_CRC, CB_DATA_TYPES } CB_DTYPE;
...
#define HEADER_MINOR(_h, _m0) ((_VER(_h, 0) >= 'A' && _VER(_h, 0) <= 'z') && (_VER(_h, 1) == (_m0)))
#define HEADER_MINOR_MORE_THAN(_h, _m0) ((_VER(_h, 0) >= 'A' && _VER(_h, 0) <= 'z') && (_VER(_h, 1) >= (_m0) && _VER(_h, 1) <= '9'))
