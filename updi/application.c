...
int app_device_info(void *app_ptr)
{
    ...
    */
    ...
    _loginfo("SIB read out as: ", sib, sizeof(sib));
    _loginfo("Family ID = ", sib, 8);
    _loginfo_i("NVM revision = %hhd", sib[10]);
    _loginfo_i("OCD revision = %hhd", sib[13]);
    _loginfo_i("PDI OSC = %hhdMHz ", sib[15]);
...
bool app_in_prog_mode(void *app_ptr)
{
    ...
    */
    ...
    status = link_ldcs(LINK(app), UPDI_ASI_SYS_STATUS);
...
int app_wait_unlocked(void *app_ptr, u16 timeout)
{
    /*   
...
int app_unlock(void *app_ptr)
{
    ...
    */
    ...
    // Check key status
    status = link_ldcs(LINK(app), UPDI_ASI_KEY_STATUS);
    if (!(status & (1 << UPDI_ASI_KEY_STATUS_CHIPERASE))) {
        _loginfo_i("Chiperase Key not accepted");
        return -3;
    }

    //Toggle reset
    app_reset(app_ptr, true);
    app_reset(app_ptr, false);
...
int app_wait_flash_ready(void *app_ptr)
{
    /*
...
int app_chip_erase(void *app_ptr)
{
    ...
    */
    ...
    result = app_wait_flash_ready(app_ptr);
    if (result) {
    ...
    //Erase
    result = app_execute_nvm_command(app_ptr, UPDI_NVMCTRL_CTRLA_CHIP_ERASE);
    if (result) {
...
int _app_write_nvm(void *app_ptr, u16 address, u8 *data, int len, u8 nvm_command, bool use_word_access)
{
    ...
    */
    ...
    result = app_execute_nvm_command(app_ptr, UPDI_NVMCTRL_CTRLA_PAGE_BUFFER_CLR);
    if (result) {
    ...
    if (use_word_access)
        ...
        result = app_write_data(app_ptr, address, data, len);
    if (result) {
    ...
    result = app_execute_nvm_command(app_ptr, nvm_command);
    if (result) {
